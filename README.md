...
Windows 原生 Codex / ChatGPT 桌面应用与 CC Switch 配置脚本。默认 KAIZO、GPT-6 Astra、Medium、Fast OFF；保留个人账户，提供通用 AGENTS.md 和脱敏日志。
...
5. 等待显示“登录状态、模型配置和实际回复验证通过”，随后打开应用，新建 Windows 本地任务。
...
桌面应用已安装时复用，缺失时从官方 Microsoft Store 安装，失败时尝试官方签名 MSIX。首次使用从官方来源下载 Windows Python 3.14.7 运行组件和需要的 CC Switch 3.20.2 安装包，按仓库清单校验 SHA-256 后才使用；下载完成后缓存，后续复用。下载和模型验证需要联网。
...
已有 ChatGPT 登录会保留，登录文件或 Windows 凭据存储方式也会保留。KAIZO 的 Key 单独写入它的供应商配置，KAIZO 模型请求使用这个 Key。只有未登录、没有已有登录凭据且使用默认文件存储的新环境，脚本才建立原生 API Key 登录，保证桌面应用能进入。
...
`/v1` 是 API 路径，不能因为 CC Switch 的端点测速失败就删除。本包以 Codex 的实际模型回复作为验证依据。CC Switch 对地址本身的 GET 测速与 `/v1/responses` 的模型调用不是同一项检查。
...
尚未在 Codex 登录个人账户时，首次启用“我的 ChatGPT 账户”需要完成官方登录。登录后再切回 KAIZO，个人登录会保留。登录失效、主动退出登录或服务器撤销授权时，仍可能需要重新登录；个人账户的网络和订阅权限按官方服务要求。如果取消了首次个人登录，之后 KAIZO 也停在登录页，可运行本包 `Repair.cmd` 重建 API Key 登录入口。
...
| 文件 | 用途 |
| --- | --- |
| `Setup.cmd` | 检查/安装应用，配置并验证，成功后打开应用 |
| `Repair.cmd` | 复用已安装且兼容的应用，重新配置、检查代理并验证 |
...
日志包含版本、Windows / PowerShell / Python 信息、每步开始与结束时间、耗时、执行程序路径与进程号、退出码、文件或注册表键路径、WinError / HRESULT、Codex 配置接口方法名、模型验证状态及回滚结果。失败步骤和最后一次操作会保留在文件中，不会随进度界面关闭而消失。
...
备份位于当前用户目录 `.kaizo-setup-backups`。验证失败时，在确认相关应用退出后自动恢复本次配置及代理修改；原本已有的登录凭据不会被安装逻辑覆盖，自动恢复也会保留它们可能发生的正常令牌刷新。若应用仍运行，保留备份并提示使用 `Restore.cmd`。手动恢复会按备份恢复登录文件。恢复不卸载已安装的软件，不撤回系统安装器或 CC Switch 自身产生的所有文件。

源码不含可用 Key。本机配置和备份含凭据，不要提交到仓库或公开 Issues；敏感文件已加入 `.gitignore`。持久凭据文件与备份目录会限制访问权限。

## 验证范围与来源

制作端已执行临时目录中的离线回归检查，覆盖已有 ChatGPT / 个人 API Key 保留、file / keyring / auto 存储模式、两张配置卡切换后新进程读取、登录异常时不覆盖凭据、令牌刷新保留，以及配置同步、重复执行、其他供应商保留、代理清理、请求超时与回滚。两张卡的切换检查是按 CC Switch 已核对的写入规则在临时文件中验证，不是 Windows CC Switch 界面实测。

本版另通过模拟验证：安装器启动拒绝后进入官方备用流程；被占用文件的错误码和路径被保留；注册表按需请求查询/写入权限；日志目录不可写时改用备用目录；步骤、错误和回滚信息持续保存；凭据与子进程原始输出不出现在日志中。PowerShell 日志写入及脱敏也已单独执行检查。

PowerShell 启动脚本及 11 段内嵌脚本已通过语法检查，代理清理段也已单独执行验证。公开版另检查隐藏输入、密钥持久保存、重复执行复用、旧配置导入、下载缓存及校验失败不运行安装器。检查仅使用虚构凭据。

本地检查不调用制作端 Mac 的 Codex 或 CC Switch。GitHub Actions 在 Windows runner 上运行组件引导和离线检查；这不等同于真实用户机器的安装、桌面交互或模型请求验证。上述真实操作只在使用者执行 Setup / Repair 时进行，失败不会标记为成功。模型验证最长等待 180 秒，进度中会显示已用时间和脱敏状态。
...
`python -X utf8 check_setup.py` 在临时目录使用虚构凭据与模拟进程回归检查；不调用真实 Codex / CC Switch 或模型 API。`scripts/check_powershell.ps1` 从当前源码提取并检查 PowerShell 语法与日志脱敏。推送会触发 Windows CI。
